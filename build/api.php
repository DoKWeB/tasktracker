...
@$_request = json_decode($_postdata);
@$_email = $_request->email;
@$_password = $_request->password;
@$_name = $_request->name;
@$_date = $_request->date;
@$_img = $_request->img;
@$_logout = $_request->logout;
@$_get_user = $_request->get_user;
...
function auth($email, $password){
  $user = check($email, $password, true);
  if($user){
...
function check($email, $password, $return = false){
  //$mysqli = new mysqli("localhost", "id5154438_root", "Qwerty1234", "id5154438_angular");
  ...
  $result_set = $mysqli->query("SELECT * FROM `task_users` WHERE `email` = '".$email."' AND `password` = '".$password."'");
...
function openAPI(){
  ...
  /*$api = new PHP_CRUD_API(array(
    'dbengine'=>'MySQL',
    'hostname'=>'localhost',
    'username'=>'id5154438_root',
    'password'=>'Qwerty1234',
    'database'=>'id5154438_angular',
    'charset'=>'utf8'
  ));*/
  //   $mysqli = new mysqli("localhost", "id5154438_root", "Qwerty1234", "id5154438_angular");
...
if($_SESSION['email'] && $_SESSION['password']){
  $user = check($_SESSION['email'], $_SESSION['password'], true);
  if($user){
    if($_get_user){
      echo $user;
      exit;
